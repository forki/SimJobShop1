...
module Schedule = 
    ...
    let addSeq schedule commands =
        Seq.fold add schedule commands

    let ofSeq commands =
        addSeq empty commands
...
type Simulation<'State, 'Command, 'Event when 'Command : comparison> =
    ...
    | SimTerminated of Id : Guid



module Simulation =
    /// Create a new simulation.
    let create id model state schedule = 
        SimRun { Id = id
                 Model = model
                 State = state
                 Schedule = schedule }
    ...
    /// for the simulation and the time of the first command as the initial time.
    let createInitial model schedule = 
        let id = Guid.NewGuid()
//        let iniTime = Schedule.getTimeOfNextCommand schedule
        create id model model.InitialState schedule


    /// Take the simulation run one time step further, meaning evaluate the next command.
    let evolveRun saveEvent log run =
    ...
    let evolveSim saveEvent log simulation =
        ...
        | SimTerminated id ->
            sprintf "Simulaten terminated: Id = %A" id |> log
            SimTerminated id
        | SimRun run -> 
