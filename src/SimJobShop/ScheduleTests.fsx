...
// Tests for Command
let ``Comparison of commands is done based on comparison of generic times`` (t1 : 'Time) (t2 : 'Time) = 
    let c1 = 
        { Time = t1
          Action = "t1" }
...
let ``Comparison of commands is done based on comparison of integer times`` (t1 : int) (t2 : int) = 
    ``Comparison of commands is done based on comparison of generic times`` t1 t2
let ``Comparison of commands is done based on comparison of float times`` (t1 : float) (t2 : float) = 
    ``Comparison of commands is done based on comparison of generic times`` t1 t2
let ``Comparison of commands is done based on comparison of DateTime times`` (t1 : System.DateTime) 
    (t2 : System.DateTime) = ``Comparison of commands is done based on comparison of generic times`` t1 t2
...
let commandTests = 
    [ ``Comparison of commands is done based on comparison of integer times`` 1 2
      ...
      ``Comparison of commands is done based on comparison of DateTime times`` System.DateTime.Today 
...
let generateSomeCommands (n : uint32) = 
    ...
    let initializer t = 
        { Time = t
          Action = (sprintf "t=%i" t) }
    Seq.init count initializer

let ``Adding n commands one by one is equivalent to adding a list of commands in the same order`` (n : uint32) = 
...
let ``Adding n commands and taking n commands yields an empty schedule`` (n : uint32) = 
    ...
    let removeOneCommand schedule = 
        ...
        newSchedule
    for i in 1..(int) n do
        s <- removeOneCommand s
    Schedule.isEmpty s

let ``Adding two commands to an empty schedule then taking one command yields the earlier or first command`` (t1 : int) 
    (t2 : int) = 
    let c1 = 
        { Time = t1
          Action = "first" }
