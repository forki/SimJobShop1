...
type DummyState = {
    Time : int
    Number : int }
type DummyAction = Add | Subtract
type DummyCommand = Command<int, DummyAction>
type DummyFact = Added | Subtracted
let dummyExecute (state:DummyState) (command:DummyCommand) =
    match command with
    | {Time=time; Action=Add} -> [{Time=time; Fact=Added}]
    | {Time=time; Action=Subtract} -> [{Time=time; Fact=Subtracted}]
let dummyApply (state:DummyState) event =
    let newState =
        match event with
        | {Time=time; Fact=Added} -> {state with Time = time; Number = state.Number + 1}
        | {Time=time; Fact=Subtracted} -> {state with Time = time; Number = state.Number - 1}
    (newState, [])
let dummyModel = {
    InitialState = {Time=0; Number=0}
